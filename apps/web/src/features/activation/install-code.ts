...
export function installCode(experiment: Experiment) {
  const lines = ['<!-- Uplift -->'];
  if (experiment.hidePage.enabled) {
    // Hides the page until the script has applied the variant, or the timeout passes.
    lines.push(
      '<style>.uplift-hide{opacity:0!important}</style>',
      `<script>(function(d,t){d.documentElement.classList.add('uplift-hide');setTimeout(function(){d.documentElement.classList.remove('uplift-hide')},t)})(document,${experiment.hidePage.timeoutMs})</script>`,
    );
  }
